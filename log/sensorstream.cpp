...
SensorReading* SensorStream::parseReading(std::istream& is, const SensorMap& smap) {
  ...
  if (is) {
    ...
    if (odometry) reading = parseOdometry(lis, odometry);

    RangeSensor* range = dynamic_cast<RangeSensor*>(sensor);
    if (range) reading = parseRange(lis, range);
  }
  return reading;
}
...
RangeReading* SensorStream::parseRange(std::istream& is, const RangeSensor* rs) {
  ...
  if (rs->newFormat) {
    string laser_type, start_angle, field_of_view, angular_resolution, maximum_range, accuracy,
        remission_mode;
    is >> laser_type >> start_angle >> field_of_view >> angular_resolution >> maximum_range >>
  ...
  if (rs->newFormat) {
    ...
    is >> reflectionBeams;
    double reflection;
    for (int i = 0; i < reflectionBeams; i++) is >> reflection;
  }
...
LogSensorStream::operator bool() const { return m_cursor == m_log->end(); }
...
InputSensorStream::operator bool() const { return (bool)m_inputStream; }
