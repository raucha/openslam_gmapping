...
void QGraphPainter::resizeEvent(QResizeEvent* sizeev) { m_pixmap->resize(sizeev->size()); }

QGraphPainter::~QGraphPainter() { delete m_pixmap; }

void QGraphPainter::clear() { values.clear(); }

void QGraphPainter::valueAdded(double v) { values.push_back(v); }
...
void QGraphPainter::disableYReference() { m_useYReference = false; }

void QGraphPainter::setTitle(const char* t) { title = t; }
...
void QGraphPainter::setAutoscale(bool a) { autoscale = a; }

bool QGraphPainter::getAutoscale() const { return autoscale; }

void QGraphPainter::timerEvent(QTimerEvent* te) {
  if (te->timerId() == timer) update();
}

void QGraphPainter::start(int period) { timer = startTimer(period); }
...
void QGraphPainter::paintEvent(QPaintEvent*) {
  ...
  for (unsigned int i = 1; i < (unsigned int)width() - 2 * boundary - xoffset && i < values.size();
       i++) {
    int v = (int)(scale * (values[i + start] - _min)) + boundary;
    painter.drawLine(i - 1 + boundary + xoffset, height() - boundary - oldv, xoffset + i + boundary,
                     height() - boundary - v);
    oldv = v;
  }
  painter.setPen(Qt::black);
