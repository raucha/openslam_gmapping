...
int GridSlamProcessorThread::init(int argc, const char* const* argv) {
  ...
  if (configfilename.length() > 0) {
    ...
    linearOdometryReliability =
        cfg.value("gfs", "linearOdometryReliability", linearOdometryReliability);
    angularOdometryReliability =
        cfg.value("gfs", "angularOdometryReliability", angularOdometryReliability);
    ebuf = (std::string)cfg.value("gfs", "estrategy", ebuf);
    considerOdometryCovariance =
        cfg.value("gfs", "considerOdometryCovariance", considerOdometryCovariance);
  }
...
void* GridSlamProcessorThread::fastslamthread(GridSlamProcessorThread* gpt) {
  if (!gpt->input && !gpt->onLine) return 0;
...
// if started online retrieve the settings from the connection
  ...
  gpt->setMatchingParameters(gpt->maxUrange, gpt->maxrange, gpt->sigma, gpt->kernelSize, gpt->lstep,
                             gpt->astep, gpt->iterations, gpt->lsigma, gpt->ogain, gpt->lskip);

  double xmin = gpt->xmin, ymin = gpt->ymin, xmax = gpt->xmax, ymax = gpt->ymax;

  OrientedPoint initialPose(0, 0, 0);

  if (gpt->autosize) {
    if (gpt->readFromStdin || gpt->onLine) cout << "Error, cant autosize form stdin" << endl;
  ...
  if (gpt->infoStream()) {
    gpt->infoStream() << " initialPose=" << initialPose.x << " " << initialPose.y << " "
                      << initialPose.theta << cout << " xmin=" << xmin << " ymin=" << ymin
...
#define printParam(n) gpt->outputStream() << "PARAM " << #n << " " << gpt->n << endl
...
#undef printParam
  ...
  if (gpt->randseed != 0) sampleGaussian(1, gpt->randseed);
  if (!gpt->infoStream()) {
  ...
  if (!gpt->onLine) {
    while (*(gpt->input) && gpt->running) {
      ...
      if (rr && gpt->running) {
        ...
        // pmap=m_gsp->getParticles()[0].map.toDoubleMap() ;
        // 					cerr << "deleting" << endl;
        // 					delete m_gsp;
        // 					delete pmap;
        // 				}
  ...
  DoneEvent* done = new DoneEvent;
  gpt->addEvent(done);
  gpt->infoStream() << "Hallo, I am the gsp thread. I have finished. Do you think it is the case "
                       "of checking for unlocked mutexes." << endl;
  return 0;
}
...
void GridSlamProcessorThread::onOdometryUpdate() {
  ...
  hypotheses.clear();
  weightSums.clear();
  for (GridSlamProcessor::ParticleVector::const_iterator part = getParticles().begin();
       part != getParticles().end(); part++) {
...
OrientedPoint GridSlamProcessorThread::boundingBox(SensorLog* log, double& xmin, double& ymin,
