...
QParticleViewer::QParticleViewer(QWidget* parent, const char* name, WFlags f,
...
void QParticleViewer::resizeEvent(QResizeEvent* sizeev) {
  if (!m_pixmap) return;
  cerr << "QParticleViewer::resizeEvent" << sizeev->size().width() << " " << sizeev->size().height()
       << endl;
...
void QParticleViewer::drawFromFile() {
  ...
  if (recordType == "ODO_UPDATE") {
    ...
    if (m_particleSize) m_refresh = true;
    m_oldPose = m_newPose;
    m_newPose.clear();
    unsigned int size;
    lineStream >> size;
    if (!m_particleSize) m_particleSize = size;
...
void QParticleViewer::drawFromMemory() {
  if (!gfs_thread) return;
  m_pixmap->fill(Qt::white);
  GridSlamProcessorThread::EventDeque events = gfs_thread->getEvents();
  for (GridSlamProcessorThread::EventDeque::const_iterator it = events.begin(); it != events.end();
       it++) {
  ...
  while (!particleSize && it != history.rend()) {
    GridSlamProcessorThread::ParticleMoveEvent* move =
        dynamic_cast<GridSlamProcessorThread::ParticleMoveEvent*>(*it);
    GridSlamProcessorThread::ResampleEvent* resample =
        dynamic_cast<GridSlamProcessorThread::ResampleEvent*>(*it);
    if (move) particleSize = move->hypotheses.size();
    if (resample) particleSize = resample->indexes.size();
    it++;
  }
  ...
  for (unsigned int i = 0; i < particleSize; i++) {
    ...
    bool done = false;
    for (GridSlamProcessorThread::EventDeque::reverse_iterator it = history.rbegin();
         it != history.rend() && !done; it++) {
      GridSlamProcessorThread::ParticleMoveEvent* move =
          dynamic_cast<GridSlamProcessorThread::ParticleMoveEvent*>(*it);
  ...
  for (unsigned int i = 0; i < particleSize + 1; i++) {
    ...
    for (GridSlamProcessorThread::EventDeque::reverse_iterator it = history.rbegin();
      ...
      if (move) {
        ...
        first = false;
        if (!(showPaths || showBestPath && i == particleSize)) break;
        pnew = pold;
      }
      GridSlamProcessorThread::ResampleEvent* resample =
          dynamic_cast<GridSlamProcessorThread::ResampleEvent*>(*it);
...
void QParticleViewer::start(int period) { timer = startTimer(period); }
...
void QParticleViewer::start() {
  gfs_thread->setMatchingParameters(matchingParameters.urange, matchingParameters.maxrange,
                                    matchingParameters.ssigma, matchingParameters.ksize,
                                    matchingParameters.lstep, matchingParameters.astep,
                                    matchingParameters.iterations, startParameters.lsigma,
                                    startParameters.lgain, startParameters.lskip);
  gfs_thread->setMotionModelParameters(startParameters.srr, startParameters.srt,
                                       startParameters.srt, startParameters.stt);
  ...
  ((GridSlamProcessor*)(gfs_thread))
      ->init(startParameters.particles, startParameters.xmin, startParameters.ymin,
             startParameters.xmax, startParameters.ymax, startParameters.delta,
...
void QParticleViewer::stop() { gfs_thread->stop(); }
