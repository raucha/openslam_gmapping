...
void computeBoundingBox(double& xmin, double& ymin, double& xmax, double& ymax,
                        const LaserRecord& laser, const OrientedPoint& pose, double maxrange) {
  double theta = -M_PI / 2 + pose.theta;
  double theta_step =
      (laser.readings.size() == 180 || laser.readings.size() == 181) ? M_PI / 180 : M_PI / 360;
  for (std::vector<double>::const_iterator it = laser.readings.begin(); it != laser.readings.end();
       it++) {
    if (*it < maxrange) {
...
void computeBoundingBox(double& xmin, double& ymin, double& xmax, double& ymax,
                        const RecordList& rl, double maxrange) {
  xmin = ymin = MAXDOUBLE;
  xmax = ymax = -MAXDOUBLE;
  ...
  for (RecordList::const_iterator it = rl.begin(); it != rl.end(); it++) {
    ...
    if (smr && lastLaser) {
      for (std::vector<OrientedPoint>::const_iterator pit = smr->poses.begin();
           pit != smr->poses.end(); pit++) {
...
int main(int argc, char** argv) {
  ...
  for (RecordList::const_iterator it = rl.begin(); it != rl.end(); it++) {
    ...
    if (!s) continue;
    scanCount++;
    if (scanCount % scanSkip) continue;
    cout << "Frame " << frame << " ";
    ...
    for (RecordList::const_iterator mt = paths[bestIdx].begin(); mt != paths[bestIdx].end(); mt++) {
      ...
      if (s) {
        ...
        for (uint i = 0; i < s->readings.size(); i++) rawreadings[i] = s->readings[i];
