...
void QSLAMandNavWidget::mousePressEvent(QMouseEvent* e) {
  ...
  if (e->state() & Qt::ShiftButton && e->button() == Qt::LeftButton) {
    if (trajectorySent) trajectoryPoints.clear();
    e->accept();
...
void QSLAMandNavWidget::keyPressEvent(QKeyEvent* e) {
  if (e->key() == Qt::Key_Delete) {
    e->accept();
    if (!trajectoryPoints.empty()) trajectoryPoints.pop_back();
  }
  if (e->key() == Qt::Key_S) {
    e->accept();
...
void QSLAMandNavWidget::paintEvent(QPaintEvent*) {
  ...
  if (trajectorySent) painter.setPen(Qt::red);
  bool first = true;
  int oldx = 0, oldy = 0;
  // paint the path
  for (std::list<IntPoint>::const_iterator it = trajectoryPoints.begin();
       it != trajectoryPoints.end(); it++) {
    int x = it->x;
    int y = height() - it->y;
    if (!first) painter.drawLine(oldx, oldy, x, y);
    oldx = x;
    oldy = y;
    first = false;
  }
  ...
  if (drawRobot) {
    ...
    int robotSize = 6;
    painter.drawLine(rx, ry, rx + (int)(robotSize * cos(robotHeading)),
                     ry - (int)(robotSize * sin(robotHeading)));
