...
void CommentRecord::write(ostream& os) { os << "#GFS_COMMENT: " << text << endl; }

PoseRecord::PoseRecord(bool ideal) { truePos = ideal; }
...
void EntropyRecord::write(ostream& os) {
  os << setiosflags(ios::fixed) << setprecision(6) << "ENTROPY " << poseEntropy << " "
     << trajectoryEntropy << " " << mapEntropy;
  os << " " << time << " pippo " << time << endl;
}
...
void RecordList::printLastParticles(ostream& os) const {
  ...
  if (!scanmatch) return;
  for (vector<OrientedPoint>::const_iterator it = scanmatch->poses.begin();
       it != scanmatch->poses.end(); it++) {
    os << "MARKER [color=black; circle=" << it->x * 100 << "," << it->y * 100 << ",10] 0 pippo 0"
       << endl;
  }
}

void RecordList::destroyReferences() {
  for (RecordList::iterator it = begin(); it != end(); it++) delete (*it);
}
...
void RecordList::printPath(ostream& os, unsigned int i, bool err, bool rawodom) const {
  ...
  for (RecordList::iterator it = rl.begin(); it != rl.end(); it++) {
    ...
    if (neffr) neff = neffr->neff / (double)sampleSize;
      ...
      if (pos) {
        if (pos->truePos) {
          ...
          if (tpf) {
            ...
            if (!err) os << "# ERROR ";
            os << neff << " " << ex << " " << ey << " " << eth << " " << sqrt(ex * ex + ey * ey)
               << " " << fabs(eth) << endl;
    ...
    PoseRecord* pos = dynamic_cast<PoseRecord*>(*it);
    if (pos) oldPose = pos->pose;
    if (!err) (*it)->write(os);
    delete *it;
  }
  if (err) cout << "average error" << totalError / count << endl;
}
