...
GridSlamProcessor::TNodeVector GridSlamProcessor::getTrajectories() const {
  ...
  for (ParticleVector::const_iterator it = m_particles.begin(); it != m_particles.end(); it++) {
    ...
    if (newnode->parent) {
      ...
      // cerr << __PRETTY_FUNCTION__ << ": node " << newnode->parent << " flag=" <<
      // newnode->parent->flag<< endl;
  ...
  while (!border.empty()) {
    ...
    // cerr << __PRETTY_FUNCTION__ << ": parentCache.size(PREPROCESS)=" << parentCache.size() <<
    // endl;
    ...
    // cerr << __PRETTY_FUNCTION__ << ": parentCache.size(POSTERASE)=" << parentCache.size() <<
    // endl;
...
void GridSlamProcessor::integrateScanSequence(GridSlamProcessor::TNode* node) {
  ...
  if (m_infoStream) m_infoStream << "Restoring State Nodes=" << count << endl;
  ...
  while (aux != 0) {
    ...
    for (unsigned int i = 0; i < m_beams; i++) plainReading[i] = (*(aux->reading))[i];
