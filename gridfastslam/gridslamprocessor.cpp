...
void GridSlamProcessor::setMatchingParameters(double urange, double range, double sigma,
                                              ...
                                              int iterations, double likelihoodSigma,
  ...
  m_matcher.setMatchingParameters(urange, range, sigma, kernsize, lopt, aopt, iterations,
                                  likelihoodSigma, likelihoodSkip);
  if (m_infoStream)
    m_infoStream << " -maxUrange " << urange << " -maxUrange " << range << " -sigma     " << sigma
                 << " -kernelSize " << kernsize << " -lstep " << lopt << " -lobsGain "
                 << m_obsSigmaGain << " -astep " << aopt << endl;
}
...
void GridSlamProcessor::setMotionModelParameters(double srr, double srt, double str, double stt) {
  ...
  if (m_infoStream)
    m_infoStream << " -srr " << srr << " -srt " << srt << " -str " << str << " -stt " << stt
                 << endl;
}
...
void GridSlamProcessor::setSensorMap(const SensorMap& smap) {
  /*
    ...
    FIXME For now detect the readings of only the front laser, and assume its pose is in the center
    of the robot
  */
...
void GridSlamProcessor::init(unsigned int size, double xmin, double ymin, double xmax, double ymax,
  ...
  m_delta = delta;
  if (m_infoStream)
    m_infoStream << " -xmin " << m_xmin << " -xmax " << m_xmax << " -ymin " << m_ymin << " -ymax "
                 << m_ymax << " -delta " << m_delta << " -particles " << size << endl;
...
void GridSlamProcessor::processTruePos(const OdometryReading& o) {
  ...
  if (os && os->isIdeal() && m_outputStream) {
    ...
    m_outputStream << setiosflags(ios::fixed) << setprecision(6) << o.getPose().theta << " "
                   << o.getTime() << endl;
  }
}
...
bool GridSlamProcessor::processScan(const RangeReading& reading, int adaptParticles) {
  ...
  if (m_linearDistance > m_distanceThresholdCheck) {
    ...
    cerr << "Old Odometry Pose= " << m_odoPose.x << " " << m_odoPose.y << " " << m_odoPose.theta
         << endl;
  ...
  // process a scan only if the robot has traveled a given distance or a certain amount of time has
  // elapsed
