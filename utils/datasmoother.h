...
class DataSmoother {
 public:
  ...
  void add(double x, double p) {
    ...
    if (x - 3.0 * m_parzenWindow < m_from) m_from = x - 3.0 * m_parzenWindow;

    if (x + 3.0 * m_parzenWindow > m_to) m_to = x + 3.0 * m_parzenWindow;
  ...
  void integrate(double step) {
    m_lastStep = step;
    double sum = 0;
    for (double x = m_from; x <= m_to; x += step) sum += smoothedData(x) * step;
    m_int = sum;
  }

  double integral(double step, double xTo) {
    double sum = 0;
    for (double x = m_from; x <= xTo; x += step) sum += smoothedData(x) * step;
    return sum;
  }
  ...
  void sampleMultiple(std::vector<double>& samples, int num) {
    ...
    for (int i = 0; i < num; i++) randoms[i] = sampleUniformDouble(0.0, maxval);
  ...
  void gnuplotDumpSmoothedData(FILE* fp, double step) {
    for (double x = m_from; x <= m_to; x += step) fprintf(fp, "%f %f\n", x, smoothedData(x));
  }
