...
void CarmenWrapper::initializeIPC(const char* name) { carmen_ipc_initialize(1, (char**)&name); }
...
bool CarmenWrapper::start(const char* name) {
  ...
  carmen_robot_subscribe_frontlaser_message(NULL, (carmen_handler_t)robot_frontlaser_handler,
                                            CARMEN_SUBSCRIBE_LATEST);
  carmen_robot_subscribe_rearlaser_message(NULL, (carmen_handler_t)robot_rearlaser_handler,
                                           CARMEN_SUBSCRIBE_LATEST);
  carmen_simulator_subscribe_truepos_message(NULL, (carmen_handler_t)simulator_truepos_handler,
                                             CARMEN_SUBSCRIBE_LATEST);
...
const SensorMap& CarmenWrapper::sensorMap() { return m_sensorMap; }

bool CarmenWrapper::isRunning() { return m_threadRunning; }

bool CarmenWrapper::isStopped() { return stopped; }
...
OrientedPoint CarmenWrapper::getTruePos() { return m_truepos; }
...
// RangeSensor::RangeSensor(std::string name, unsigned int beams_num, unsigned int res, const
// OrientedPoint& position, double span, double maxrange):
...
void CarmenWrapper::robot_frontlaser_handler(carmen_robot_laser_message* frontlaser) {
  /*	if (! m_rangeSensor){
                  ...
                  if (frontlaser->num_readings==360 || frontlaser->num_readings==361)
                          res=M_PI/360;
                  assert(res>0);
                  m_rangeSensor=new RangeSensor("FLASER",frontlaser->num_readings, res,
     OrientedPoint(0,0,0), 0, 89.9);
...
void CarmenWrapper::navigator_go_handler(MSG_INSTANCE msgRef, BYTE_ARRAY callData, void*) {
  ...
  err = IPC_unmarshallData(formatter, callData, &msg, sizeof(carmen_navigator_go_message));
...
void CarmenWrapper::navigator_stop_handler(MSG_INSTANCE msgRef, BYTE_ARRAY callData, void*) {
  ...
  formatter = IPC_msgInstanceFormatter(msgRef);
  err = IPC_unmarshallData(formatter, callData, &msg, sizeof(carmen_navigator_stop_message));
  IPC_freeByteArray(callData);
...
RangeReading CarmenWrapper::carmen2reading(const carmen_robot_laser_message& msg) {
  ...
  if (msg.laser_pose.theta == msg.robot_pose.theta && !m_frontLaser) {
    ...
    m_frontLaser = new RangeSensor(sensorName, msg.num_readings, res,
                                   OrientedPoint(0, 0, msg.laser_pose.theta - msg.robot_pose.theta),
                                   0, msg.config.maximum_range);
    m_frontLaser->updateBeamsLookup();
    m_sensorMap.insert(make_pair(sensorName, m_frontLaser));

    cout << __PRETTY_FUNCTION__ << ": " << sensorName << " configured."
         << " Readings " << m_frontLaser->beams().size() << " Resolution " << res << endl;
  }
  if (msg.laser_pose.theta != msg.robot_pose.theta && !m_rearLaser) {
    ...
    m_rearLaser = new RangeSensor(sensorName, msg.num_readings, res,
                                  OrientedPoint(0, 0, msg.laser_pose.theta - msg.robot_pose.theta),
                                  0, msg.config.maximum_range);
    ...
    cout << __PRETTY_FUNCTION__ << ": " << sensorName << " configured."
         << " Readings " << m_rearLaser->beams().size() << " Resolution " << res << endl;
  }

  const RangeSensor* rs =
      (msg.laser_pose.theta == msg.robot_pose.theta) ? m_frontLaser : m_rearLaser;
...
void CarmenWrapper::publish_globalpos(carmen_localize_summary_p summary) {
  ...
  carmen_test_ipc_exit(err, "Could not publish", CARMEN_LOCALIZE_GLOBALPOS_NAME);
  unlock();
}
...
void CarmenWrapper::publish_particles(carmen_localize_particle_filter_p filter,
  ...
  carmen_test_ipc_exit(err, "Could not publish", CARMEN_LOCALIZE_PARTICLE_NAME);
  fprintf(stderr, "P");
  unlock();
}
