...
class GridSlamProcessor {
 public:
  /**This class defines the the node of reversed tree in which the trajectories are stored.
     Each node of a tree has a pointer to its parent and a counter indicating the number of childs
     of a node.
 ...
 */
  struct TNode {
    ...
    */
    ...
    /**Destroys a tree node, and consistently updates the tree. If a node whose parent has only one
     child is deleted,
     also the parent node is deleted. This because the parent will not be reacheable anymore in the
     trajectory tree.*/
    ~TNode();
  ...
  /**This class defines a particle of the filter. Each particle has a map, a pose, a weight and
   * retains the current node in the trajectory tree*/
  struct Particle {
    ...
    */
    ...
    /** The pose of the robot at the previous time frame (used for computing thr odometry
     * displacements) */
  ...
  */
  ...
  /**the beams to skip for computing the likelihood (consider a beam every likelihoodSkip)
   * [scanmatcher]*/
 ...
 protected:
  ...
  // registration score (if a scan score is below this threshold a scan matching failure is
  // reported)
