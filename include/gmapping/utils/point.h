...
void orientedpoint<T, A>::normalize() {
  if (theta >= -M_PI && theta < M_PI) return;
...
inline point<T> interpolate(const point<T>& p1, const F& t1, const point<T>& p2, const F& t2,
                            const F& t3) {
  F gain = (t3 - t1) / (t2 - t1);
...
inline orientedpoint<T, A> interpolate(const orientedpoint<T, A>& p1, const F& t1,
