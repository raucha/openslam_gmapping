...
if you want to look for a specific topic search for one of the following keywords in the file
comments
...
int main(int argc, const char* const* argv) {
  ...
  processor->setMatchingParameters(maxUrange, maxrange, sigma, kernelSize, lstep, astep, iterations,
                                   lsigma, ogain, lskip);
  ...
  if (outfilename.length() > 0) processor->outputStream().open(outfilename.c_str());
  ...
  while (running) {
    while (CarmenWrapper::getReading(rr)) {
      ...
      // this returns true when the algorithm effectively processes (the traveled path since the
      // last processing is over a given threshold)
      if (processed) {
        cerr << "PROCESSED" << endl;
        ...
        // this is for recovering the tree of PARTICLE TRAJECTORIES (obtaining the ancestor of each
        // particle)
