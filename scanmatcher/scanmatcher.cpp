...
/*
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p, const double*
readings){
...
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p,
  ...
  if (!map.isInside(min) || !map.isInside(max)) {
    ...
    // cerr << "BOUNDARY OVERRIDE " << min.x << " " << min.y << " " << max.x << " " << max.y <<
    // endl;
  ...
  /*
          cerr << "ActiveArea=";
          for (HierarchicalArray2D<PointAccumulator>::PointSet::const_iterator
     it=activeArea.begin(); it!= activeArea.end(); it++){
                  cerr << "(" << it->x <<"," << it->y << ") ";
          }
          cerr << endl;
  */
        ...
        for (const double* r=readings; r<readings+m_laserBeams; r++, angle++)
                if (m_generateMap){
                        ...
                        for (int i=0; i<line.num_points-1; i++){
                                ...
                                if
(map.storage().getActiveArea().find(ci)==map.storage().getActiveArea().end())
                                        cerr << "BIG ERROR" <<endl;
...
double ScanMatcher::icpOptimize(OrientedPoint& pnew, const ScanMatcherMap& map,
...
double ScanMatcher::optimize(OrientedPoint& _mean, ScanMatcher::CovarianceMatrix& _cov,
                             const ScanMatcherMap& map, const OrientedPoint& init,
  ...
  // cout << "delta.x=" << delta.x << " delta.y=" << delta.y << " delta.theta=" << delta.theta <<
  // endl;
...
double ScanMatcher::likelihood(double& _lmax, OrientedPoint& _mean, CovarianceMatrix& _cov,
...
void ScanMatcher::setMatchingParameters(double urange, double range, double sigma, int kernsize,
                                        double lopt, double aopt, int iterations,
