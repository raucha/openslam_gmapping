std::vector<unsigned int> sistematicResampler<State, Numeric>::resample(
...
void evolver<State, Numeric, EvolutionModel>::evolve(
    std::vector<evolver::Particle>& particles) const {
  for (std::vector<Particle>::const_iterator it = particles.begin(); it != particles.end(); ++it)
    *it = evolutionModel.evolve(*it);
}

void evolver<State, Numeric, EvolutionModel>::evolve(
    std::vector<evolver::Particle>& dest, const std::vector<evolver::Particle>& src) const {
  dest.clear();
  for (std::vector<Particle>::const_iterator it = src.begin(); it != src.end(); ++it)
    dest.push_back(evolutionModel.evolve(*it));
}

template <class State, class Numeric, class QualificationModel, class EvolutionModel,
...
struct auxiliaryEvolver {
  ...
  void
  auxiliaryEvolver<State, Numeric, QualificationModel, EvolutionModel, LikelyhoodModel>::evolve(
    ...
    for (std::vector<unsigned int>::const_iterator it = indexes.begin(); it != indexes.end();
         it++) {
  ...
  void auxiliaryEvolver<State, Numeric, QualificationModel, EvolutionModel,
