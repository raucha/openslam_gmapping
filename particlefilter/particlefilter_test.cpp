...
struct QualificationModel {
  Particle evolve(const Particle& p) { return p; }
};
...
int main(unsigned int argc, const char* const* argv) {
  int nparticles = 100;
  if (argc > 1) nparticles = atoi(argv[1]);
  vector<Particle> particles(nparticles);
  LikelyhoodModel likelyhoodModel;
  uniform_resampler<Particle, double> resampler;
  auxiliary_evolver<Particle, double, QualificationModel, EvolutionModel, LikelyhoodModel>
      auxevolver;
